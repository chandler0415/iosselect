...
(function() {
   ...
   * @type {{isArray: 是否是数组,
    * isFunction: 是否是函数,
    * attrToData: 把dom中的dom本身和index转换成一个obj,
    * attrToHtml: 把数据中的id和name变成一段html
    * }}
   */
  var iosSelectUtil = {
  ...
  /**
   * 部局函数，把传入的html片断放到指定的容器中，再绑定上关闭函数
   * @param html
   * @param opts
   * @return {Layer}
   * @constructor
  ...
  Layer.prototype = {
    /**
     * init方法主要做了判断是否有容器，然后给了className，height属性添加无效。然后初始化绑定事件。
     */
    init: function() {
      ...
      // note Element.getBoundingClientRect()方法返回元素的大小及其相对于视口的位置。
      // note window.innerHeight浏览器窗口的视口（viewport）高度（以像素为单位），如果存在水平滚动条，则包括它。
      // note 这段代码没有意义，只给数字时不能给上高度，并且如果docment过长时，这个高度如果真能生效，样式上一定发生灾难
      // todo delete
    ...
    // note 这个只对dom做了addEventListener，在隐藏时候，dom也被移除了，所以ok
    bindEvent: function() {
  ...
  function IosSelect(level, data, options) {
    // note [arr1, arr2] 如果传入的数据不是数组的话，返回
  ...
  IosSelect.prototype = {
    ...
    initLayer: function() {
      ...
      var all_html = [
        ...
        // todo 这里为什么不直接给出值呢，在初始化时，同样会给出title的吧
        '<h2 id="iosSelectTitle"></h2>',
        // todo 这里感觉用span也可以，还可以省去href属性设置
        ...
        '</div>',
        '</section>',
        // note 这是两条线，一上一下，感觉这两条线可以用一个dom加样式来搞定
      ...
      // todo 这里逻辑有点乱，这里给body加了hidden，上面还有一处对容器加了一个判断得出的高度（虽然那里计算的值没用），那那个值直接给body的就ok了啊。
      document.body.classList.add('ios-select-body-class');
      // todo 直接body加hidden就好，不需要再scrollTo(0,0)了吧。
      window.scrollTo(0, 0);

      this.scrollOne = new IScroll('#oneLevelContain', {
        // note http://blog.csdn.net/sweetsuzyhyf/article/details/44195549/
        // note 3代表每滚动一像素触发一次
        probeType: 3,
        bounce: false
      });
      this.scrollOne.on('scrollStart', function() {
        // todo 这里用Array.prototype.slice有什么用？
    ...
    /**
     * 这信方法主要是设置基本的单位，在这里学到了如何正确的通过原生获取dom元素的style属性
     */
    setBase: function() {
      if (this.options.cssUnit === 'rem') {
        var dltDom = document.documentElement;
        // note https://developer.mozilla.org/zh-CN/docs/Web/API/Window/getComputedStyle
        var dltStyle = window.getComputedStyle(dltDom, null);
        // note 虽然dltStyle是一个实时的对象，但baseSize的赋值会是实时的吗？
        // todo 事实证明并不会实时变换，只会在setBase方法调用时才会去触发一次，那这个api感觉没有多大意义
        // note 其实和实时没有任何关系，https://developer.mozilla.org/zh-CN/docs/Web/API/HTMLElement/style
